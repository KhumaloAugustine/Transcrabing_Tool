...
function renderEditor(){
  ...
  $("#editorTitle").textContent=current.title;$("#editorMeta").textContent=`${new Date(current.created_at).toLocaleString()} · ${current.original_name}`;$("#editorStatus").textContent=statusLabel(current.status);$("#editorStatus").className=`status-chip ${current.status==="needs_review"?"review":""}`;
  ...
  $("#transcribeBtn").style.display=current.zulu_segments.length?"none":"inline-block";renderTranscript();paintWave(0)
}
...
function renderTranscript(){
  ...
  $("#transcript").innerHTML=data.map((s,i)=>`<div class="segment" data-index="${i}"><time>${formatTime(s.start)}</time><span class="speaker">${escapeHtml(s.speaker)}</span><p contenteditable="true" spellcheck="true">${escapeHtml(s.text)}</p></div>`).join("");
...
$("#transcribeBtn").onclick=()=>runAction($("#transcribeBtn"),`/api/interviews/${current.id}/transcribe`,"Transcribing…","IsiZulu transcript created — review it against the audio");
